...
## Step 1, interview (plain words, at most four questions)
...
Pass forward, in the routing message: the locked pack id, the four locked answers, the out-of-bounds
list, and any unanswered slot **named as unanswered** so the verb opens it as a
...
- **A company with no pack, when the ask wants company-grounded output:** say plainly that a context
  ...
  generic mode meanwhile. Never fake grounding: generic mode with named gaps beats a run that quietly
